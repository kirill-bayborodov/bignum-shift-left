...
#include "adapter/bignum_shift_left_benchmark_adapter.h"

int main(int argc, char **argv)
{
    benchmark_adapter_t adapter;
    bignum_shift_left_benchmark_status_t adapter_status;
    benchmark_core_status_t core_status;

    adapter_status = bignum_shift_left_benchmark_adapter_init(&adapter);
    if (adapter_status != BIGNUM_SHIFT_LEFT_BENCHMARK_STATUS_SUCCESS) return EXIT_FAILURE;
