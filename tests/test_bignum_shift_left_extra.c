/**
 * @file    test_bignum_shift_left_extra.c
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    03.10.2025
 *
 * @brief   Расширенные тесты для модуля bignum_shift_left
 *
 * @details
 *   Проверяем:
...
static void test_null_arg(void) {
    ...
    int rc = bignum_shift_left(NULL, 10);
    assert(rc == BIGNUM_SHIFT_ERROR_NULL_ARG);
    printf("OK\n");
}
...
static void test_zero_stays_zero(void) {
    ...
    for (size_t s = 0; s < 200; s += 37) {
        ...
        int rc = bignum_shift_left(&x, s);
        assert(rc == BIGNUM_SHIFT_SUCCESS);
...
static void test_associativity(void) {
    ...
    for (size_t x = 0; x < 130; x += 17) {
        for (size_t y = 0; y < 130; y += 23) {
            ...
            size_t sum = x + y;
            int c1 = bignum_shift_left(&r1, x);
            int c2 = bignum_shift_left(&r1, y);
            int c3 = bignum_shift_left(&r3, sum);
            if (c1 == BIGNUM_SHIFT_SUCCESS && c2 == BIGNUM_SHIFT_SUCCESS) {
                assert(c1 == BIGNUM_SHIFT_SUCCESS && c2 == BIGNUM_SHIFT_SUCCESS);
                assert(bignum_are_equal(&r1, &r3));
            }
            // Если один из сдвигов обещал overflow, то суммарный тоже
            if (c1 != BIGNUM_SHIFT_SUCCESS || c2 != BIGNUM_SHIFT_SUCCESS) {
                assert(c3 == BIGNUM_SHIFT_ERROR_OVERFLOW);
            }
        }
    }
    printf("OK\n");
...
static void test_edge_shift_amounts(void) {
    ...
    for (size_t i = 0; i < sizeof(shifts)/sizeof(shifts[0]); ++i) {
        ...
        if (s >= (size_t)BIGNUM_CAPACITY*64) {
            assert(rc == BIGNUM_SHIFT_ERROR_OVERFLOW);
        } else {
            assert(rc == BIGNUM_SHIFT_SUCCESS);
...
static void test_all_ones_and_truncate(void) {
    ...
    assert(rc == BIGNUM_SHIFT_ERROR_OVERFLOW ||
           (rc == BIGNUM_SHIFT_SUCCESS && x.len <= BIGNUM_CAPACITY));
    printf("OK\n");
}
...
/*void test_associativity() {
    ...
    for (int k = 0; k < 1000; ++k) {
        ...
        bignum_shift_status_t st1_a = bignum_shift_left(&num1, shift1);
        if (st1_a == BIGNUM_SHIFT_SUCCESS) {
            bignum_shift_left(&num1, shift2);
        }

        bignum_shift_status_t st2 = bignum_shift_left(&num2, total_shift);
...
int main(void) {
    printf("=== Extra tests for bignum_shift_left 1.0.0 ===\n");
    test_null_arg();
